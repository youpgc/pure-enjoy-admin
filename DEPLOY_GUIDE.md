...
version: 1.9.3+45
# 格式: 主版本.次版本.修订号+构建号
# 示例: v1.9.3+45 → v1.9.4+46 (修订号+1, 构建号+1)
# 示例: v1.9.3+45 → v1.10.0+46 (次版本+1, 修订号归零, 构建号+1)
# 示例: v1.9.3+45 → v2.0.0+46 (主版本+1, 其他归零, 构建号+1)
```

**版本号递增规则** (`v{主版本}.{次版本}.{修订号}+{构建号}`):

| 更新类型 | 示例 | 说明 |
|---------|------|------|
| **Bug 修复** | `v1.9.3+45` → `v1.9.4+46` | 修订号 +1，构建号 +1 |
| **新功能** | `v1.9.3+45` → `v1.10.0+46` | 次版本 +1，修订号归零，构建号 +1 |
| **重大更新** | `v1.9.3+45` → `v2.0.0+46` | 主版本 +1，其他归零，构建号 +1 |
| **仅重新构建** | `v1.9.3+45` → `v1.9.4+46` | 修订号必须 +1，构建号 +1 |

- **主版本号**：重大功能更新、架构变更、不兼容改动
- **次版本号**：新功能添加、功能增强
- **修订号**：Bug 修复、小优化
- **构建号**：每次构建自动递增（CI/CD 使用）

### 更新版本号步骤

1. **确定更新类型**，按规则修改版本号
2. 修改 `pubspec.yaml` 中的版本号
3. 提交代码
4. 构建 APK
5. 上传 APK 到服务器/CDN
...
# Bug 修复: 1.9.3+45 → 1.9.4+46
sed -i 's/version: 1.9.3+45/version: 1.9.4+46/' pubspec.yaml

# 或新功能: 1.9.3+45 → 1.10.0+46
sed -i 's/version: 1.9.3+45/version: 1.10.0+46/' pubspec.yaml
```
